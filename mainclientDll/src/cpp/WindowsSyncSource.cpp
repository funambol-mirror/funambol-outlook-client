...
int WindowsSyncSource::beginSync() {
    ...
    try {
        ...
        sprintf(lastErrorMsg, ERR_BEGIN_SYNC, getName());
        manageSourceError(ERR_CODE_OPEN_OUTLOOK, lastErrorMsg);
    ...
    try {
        ...
        // Must set the errors, here could be a fatal exception
        sprintf(lastErrorMsg, ERR_BEGIN_SYNC, getName());
        manageSourceError(ERR_CODE_READ_ALL_ITEMS, lastErrorMsg);
        report->setState(SOURCE_ERROR);
        manageClientException(e);
        goto error;
    }
...
int WindowsSyncSource::endSync() {
    ...
    if (!oldItemsPath) {
        manageSourceError(ERR_CODE_OLD_ITEMS_PATH, lastErrorMsg);
        return 1;
    }
...
error:
    sprintf(lastErrorMsg, ERR_END_SYNC, getName());
...
SyncItem* WindowsSyncSource::getFirstItem() {
    ...
    fireSyncSourceEvent(config.getURI(), config.getName(), getSyncMode(), noc, SYNC_SOURCE_TOTAL_CLIENT_ITEMS);
    ...
    if (allItems.size() > 0) {
        ...
        try {
            ...
            manageClientException(e);
            sprintf(lastErrorMsg, ERR_ITEM_GET, getSafeItemName(cItem).c_str(), getName());
            manageSourceError(ERR_CODE_ITEM_GET, lastErrorMsg);
            // TBD: Something wrong -> getNextItem and update report/fire events. 
            return NULL;
        }
        iAll++;
...
SyncItem* WindowsSyncSource::getFirstNewItem() {
    ...
    if (newItems.size() > 0) {
        ...
        try {
            ...
            manageClientException(e);
            sprintf(lastErrorMsg, ERR_ITEM_GET_NEW, getSafeItemName(cItem).c_str(), getName());
            manageSourceError(ERR_CODE_ITEM_GET, lastErrorMsg);
            return NULL;
        }
        iNew++;
...
SyncItem* WindowsSyncSource::getFirstUpdatedItem() {
    ...
    if (modItems.size() > 0) {
        ...
        try {
            ...
            manageClientException(e);
            sprintf(lastErrorMsg, ERR_ITEM_GET_MOD, getSafeItemName(cItem).c_str(), getName());
            manageSourceError(ERR_CODE_ITEM_GET, lastErrorMsg);
            return NULL;
        }
        iMod++;
        ...
        while (iAll != allItems.end()) {
            ...
            try {
                ...
                if (cItem->deleteItem()) {
                    ...
                    sprintf(lastErrorMsg, ERR_ITEM_DELETE, getName(), savedSubject.c_str(), cItem->getParentPath().c_str());
                    manageSourceError(ERR_CODE_DELETE, lastErrorMsg);
                }
                else {
                    // OK
...
//
errorFolder:
    sprintf(lastErrorMsg, ERR_FOLDER_OPEN, getName(), path.c_str());
    manageSourceError(ERR_CODE_FOLDER_OPEN, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

errorFolderPath:
    sprintf(lastErrorMsg, ERR_FOLDER_PATH, path.c_str(), getName());
    manageSourceError(ERR_CODE_FOLDER_PATH, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

errorCreate:
    sprintf(lastErrorMsg, ERR_ITEM_CREATE, getName(), folder->getPath().c_str());
    manageSourceError(ERR_CODE_ITEM_CREATE, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

errorFillItem:
    sprintf(lastErrorMsg, ERR_ITEM_FILL, getName(), getSafeItemName(cItem).c_str());
    manageSourceError(ERR_CODE_ITEM_FILL, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

//errorBadType:
//    sprintf(lastErrorMsg, ERR_ITEM_BAD_TYPE, item.getDataType());
//    manageSourceError(ERR_CODE_ITEM_BAD_TYPE, lastErrorMsg);
//    ret = UNSUPPORTED_MEDIA_TYPE;
//    goto finally;

errorSave:
    sprintf(lastErrorMsg, ERR_ITEM_SAVE, getName(), getSafeItemName(cItem).c_str(), folder->getPath().c_str());
    manageSourceError(ERR_CODE_ITEM_SAVE, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;
...
//
errorNotFound:
    sprintf(lastErrorMsg, ERR_ITEM_UPDATE_NOT_FOUND, getName(), item.getKey());
    manageSourceError(ERR_CODE_UPDATE_NOT_FOUND, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

errorFolderPath:
    sprintf(lastErrorMsg, ERR_FOLDER_PATH, cItem->getParentPath().c_str(), getName());
    manageSourceError(ERR_CODE_FOLDER_PATH, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;
...
errorDelete:
    sprintf(lastErrorMsg, ERR_ITEM_DELETE, getName(), getSafeItemName(cItem).c_str(), cItem->getParentPath().c_str());
    manageSourceError(ERR_CODE_DELETE, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;

errorSave:
    sprintf(lastErrorMsg, ERR_ITEM_SAVE, getName(), getSafeItemName(cItem).c_str(), cItem->getParentPath().c_str());
    manageSourceError(ERR_CODE_ITEM_SAVE, lastErrorMsg);
    ret = STC_COMMAND_FAILED;
    goto finally;
...
// 
errorNotFound:
    sprintf(lastErrorMsg, ERR_ITEM_DELETE_NOT_FOUND, getName(), item.getKey());
    manageSourceError(ERR_CODE_DELETE_NOT_FOUND, lastErrorMsg);
...
int WindowsSyncSource::manageModificationsFromLastSync() {
    ...
    if (getElementContent(fileContent, L"LastSyncTime", lastSyncTime)) {
        sprintf(lastErrorMsg, ERR_SOURCE_LASTSYNCTIME_NOT_FOUND, oldItemsPath);
        LOG.error(lastErrorMsg);
        goto error;
    }
    ...
    if (start != wstring::npos) {
        ...
        if (!f) {
            ...
            if (!f) {
                ...
                manageSourceError(ERR_CODE_NO_ROOT_FOLDER, lastErrorMsg);
                return false;
            }
