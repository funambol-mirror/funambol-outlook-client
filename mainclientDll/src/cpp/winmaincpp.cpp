...
int startSync() {
    ...
    if(!SetThreadPriority(syncThread, priority)) {
        ...
        sprintf (lastErrorMsg, ERR_THREAD_PRIORITY, code, msg);
        delete [] msg;
    ...
    if(!syncMutex){
        ...
        sprintf(lastErrorMsg, ERR_MUTEX_CREATE, msg);
        LOG.error(lastErrorMsg);
        delete [] msg;
        ret = 1;
        goto finally;
    ...
    if(GetLastError() == ERROR_ALREADY_EXISTS) {
        char* msg = readSystemErrorMsg();
        sprintf(lastErrorMsg, ERR_MUTEX_ALREADY_EXISTS, msg);
        LOG.error(lastErrorMsg);
        delete [] msg;
        ret = 1;
        goto finally;
    ...
    if(report){  // check if the sync report is valid
        ...
        //ret = lastErrorCode = report->getLastErrorCode();
        //sprintf(lastErrorMsg, "%s", report->getLastErrorMsg());
...
void endSync() {
    ...
    if (syncMutex) {
        ...
        if (!ReleaseMutex(syncMutex)) {
            char* msg = readSystemErrorMsg();
            sprintf(lastErrorMsg, ERR_MUTEX_NOT_RELEASED, msg);
            LOG.error(lastErrorMsg);
            delete [] msg;
        }
...
bool checkSyncInProgress() {
    ...
    if (hMutex == NULL) {
        ...
        // Some other error
        char* msg = readSystemErrorMsg();
        sprintf(lastErrorMsg, ERR_MUTEX_OPEN, msg);
        LOG.error(lastErrorMsg);
        delete [] msg;
        return false;
    }
...
int hardTerminateSync(HANDLE hSyncThread) {
    ...
    if (!TerminateThread(hSyncThread, 4)) {
        char* msg = readSystemErrorMsg();
        sprintf(lastErrorMsg, ERR_THREAD_NOT_TERMINATED, msg);
        LOG.error(lastErrorMsg);
        delete [] msg;
        return 1;
    }

    sprintf(lastErrorMsg, INFO_SYNC_ABORTED_BY_USER);
...
void checkAbortedSync() {
    ...
    if (config->getAbortSync()) {
        ...
        LOG.info(lastErrorMsg);
        config->setAbortSync(false);
        throwSyncException(lastErrorMsg, 2);
    }
}
