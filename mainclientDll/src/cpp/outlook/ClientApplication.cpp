...
void ClientApplication::setTimezone(ClientAppointment* cApp) {
    ...
    if (!getDisplayTimezone(tz, &display)) {
        ...
        LOG.error("Possible timezone error in the just created appointment");
    }
...
/*
void ClientApplication::setTimezone(_AppointmentItemPtr ptr) {
       
    REG_TZI_FORMAT* tz = new REG_TZI_FORMAT();
    memset(tz, 0, sizeof(REG_TZI_FORMAT));
    StringBuffer display;
    
    readMyTimezone(tz, &display);    

    int size    = sizeof(*tz);    
    char* timez = new char[size + 1];
    memcpy(timez, tz, size);
    timez[size] = 0;       
    ...
    // set the timezone blob
    long timezone_bin = pRedUtils->GetIDsFromNames(ptr->MAPIOBJECT, "{00062002-0000-0000-C000-000000000046}", 
                                        0x8233, VARIANT_TRUE);
   
    timezone_bin = timezone_bin | 0x0102; // binary
    
    pRedUtils->HrSetOneProp(ptr, timezone_bin, variant_timez, VARIANT_TRUE);   

    // set the timezone string information
    long timezone_display = pRedUtils->GetIDsFromNames(ptr->MAPIOBJECT, "{00062002-0000-0000-C000-000000000046}", 
                                        0x8234, VARIANT_TRUE);
    ...
    pRedUtils->HrSetOneProp(ptr, timezone_display, variant_display, VARIANT_TRUE);           
    
}



void readMyTimezone(REG_TZI_FORMAT* tz, StringBuffer* display) {
  
   std::wstring standardName(L"ora solare Europa occidentale");
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TIMEZONE_CONTEXT, 0, KEY_READ, &hkTimeZones) == ERROR_SUCCESS) {
        ...
        while (RegEnumKey(hkTimeZones, dwIndex++, keyName, keyNameLenght) != ERROR_NO_MORE_ITEMS) {
            if (RegOpenKeyEx(hkTimeZones, keyName, 0, KEY_READ, &hkTimeZone) == ERROR_SUCCESS) {
                ...
                if (!wcscmp(stdName, standardName.c_str())) {
                    found = true;
                    
                    // Get TZ
                    dwDataSize = sizeof(REG_TZI_FORMAT);
                    RegQueryValueEx(hkTimeZone, L"TZI", NULL, NULL, (BYTE*)tz, &dwDataSize);
