...
int ClientAppException::read() {
    ...
    if (pException) {
        try {
            ...
            manageComErrors(e);
            sprintf(lastErrorMsg, ERR_OUTLOOK_EXC_PROP_VALUE, L"OriginalDate");
    ...
    try {
        ...
        if (FAILED(hr)) {
            sprintf(lastErrorMsg, ERR_OUTLOOK_EXC_PROP_VALUE, L"Body");
            throwClientException(lastErrorMsg);
        }
        if (tmpVal) {
            body = tmpVal;
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_EXC_PROP_VALUE, exAppointmentFields[i]);
    throwClientException(lastErrorMsg);
    return 1;
}
...
int ClientAppException::saveOccurrence() {
    ...
    try {
        ...
        if (!len) {
            // this is required!
            sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE_REQUIRED, L"Start", L"appointment exception");
            return 1;
        }
        ...
        len = end.length();
         if (!len) {
            // this is required!
            sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE_REQUIRED, L"End", L"appointment exception");
            return 1;
        } 
...
errorPut:
    sprintf(lastErrorMsg, ERR_OUTLOOK_EXAPP_PROP_SET, exAppointmentFields[i]);
    throwClientException(lastErrorMsg);
    return 1;
