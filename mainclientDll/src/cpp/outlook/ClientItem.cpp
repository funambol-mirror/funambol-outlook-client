...
void ClientItem::createPropertyMap() {
    ...
    try {
        ...
        manageComErrors(e);
        sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_MAP, itemType.c_str(), i);
...
error:
    throwClientException(lastErrorMsg);         // TBD: comment here to avoid dropping item at first wrong prop...
    return ret;
}
