...
const int ClientFolder::getSubfoldersCount() {
    ...
    try {
        ...
        manageComErrors(e);
        sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_COUNT, name.c_str());
...
const int ClientFolder::getItemsCount() {
   try {
        ...
        manageComErrors(e);
        sprintf(lastErrorMsg, ERR_OUTLOOK_ITEMS_COUNT, name.c_str());
        throwClientException(lastErrorMsg);
    }
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_NOT_FOUND, 0, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_NOT_FOUND, subfoldersIndex+1, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_NOT_FOUND, subfoldersCount-1, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_NOT_FOUND, index, name.c_str());
    //throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    // if not found
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_NAME, subName.c_str(), name.c_str());
    //throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_SUBFOLDER_CREATE, subName.c_str(), type.c_str(), name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_NOT_FOUND, 0, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_NOT_FOUND, itemsIndex+1, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_NOT_FOUND, itemsCount-1, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_CREATE, itemsCount, name.c_str());
    throwClientException(lastErrorMsg);
    return NULL;
}
...
badItem:
    ...
    if (isDList && itemType == CONTACT) {
        ...
        if (isMail) {
            ...
            sprintf(lastErrorMsg, "%s", ERR_OUTLOOK_BAD_ITEM_MSG);
        }
        return NULL;
    }
    else {
