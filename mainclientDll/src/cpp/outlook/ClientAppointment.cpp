...
error: 
    sprintf(lastErrorMsg, ERR_OUTLOOK_CLEAR_REC, itemType.c_str(), getSafeItemName(this).c_str());
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_SAVE, itemType.c_str());
    throwClientException(lastErrorMsg);
    return 1;
}
...
error:
    sprintf(lastErrorMsg, ERR_OUTLOOK_ITEM_DELETE, itemType.c_str(), ID.c_str());
    throwClientException(lastErrorMsg);
    return 1;
}
...
const wstring ClientAppointment::getSafeProperty(const wstring& propertyName) {
    ...
    if (FAILED(hr)) {
        sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE, itemType.c_str(), propertyName.c_str());
        throwClientException(lastErrorMsg);
    }
    if (tmpVal) {
...
const wstring ClientAppointment::getComplexProperty(const wstring& propertyName) {
    ...
    // Get ReminderSoundFile ONLY IF ReminderPlaySound is true
        ...
        sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_NOT_FOUND, propertyName.c_str(), itemType.c_str());
        throwClientException(lastErrorMsg);
...
int ClientAppointment::setComplexProperty(const wstring& propertyName, const wstring& propertyValue) {
    ...
    if (propertyName == L"Start") {
        if (!len) {
            // this is required!
            sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE_REQUIRED, L"Start", itemType.c_str());
            return 1;
        }
        ...
        if (!len) {
            // this is required!
            sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE_REQUIRED, L"End", itemType.c_str());
            return 1;
        } 
        systemTimeToDouble(propertyValue, &date);
    ...
    if (FAILED(hr)) {
        sprintf(lastErrorMsg, ERR_OUTLOOK_PROP_VALUE_SET, propertyName.c_str(), propertyValue.c_str(), itemType.c_str());
        throwClientException(lastErrorMsg);
        ret = 1;
    }
    return ret;
