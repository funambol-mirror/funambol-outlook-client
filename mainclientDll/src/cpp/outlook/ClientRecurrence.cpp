...
int ClientRecurrence::read() {
    ...
    if (!pRec || !isRecurring()) {
        sprintf(lastErrorMsg, ERR_OUTLOOK_REC_NOT_SET, L"read");
...
int ClientRecurrence::save() {
    ...
    // Check COM Ptr / Recurrence active
    if (!pRec || !isRecurring()) {
        sprintf(lastErrorMsg, ERR_OUTLOOK_REC_NOT_SET, L"save");
        throwClientException(lastErrorMsg);
        return 1;
    }
...
error:
    ...
    if (i>=0 && recurrenceProps[i]) {
        sprintf(lastErrorMsg, ERR_OUTLOOK_REC_SAVE, recurrenceProps[i]);
...
const wstring ClientRecurrence::getProperty(const wstring& propertyName) {
    ...
    if (propertyName == L"RecurrenceType") {
        ...
        sprintf(lastErrorMsg, ERR_OUTLOOK_REC_PROP_NOT_FOUND, propertyName.c_str());
        throwClientException(lastErrorMsg);
...
int ClientRecurrence::saveException(ClientAppException* cException) {
    ...
    if (!originalDate) { 
        ...
        sprintf(lastErrorMsg, ERR_PROPERTY_REQUIRED, L"appointment exception", L"OriginalDate");
        LOG.error(lastErrorMsg);
        return 1;
    }
    ...
    if (!pOcc) {
        ...
        // sprintf(lastErrorMsg, ERR_OCCURRENCE_NOT_FOUND, cException->formatOriginalDate().c_str());
    ...
    if (cException->getDeleted()) {
        if (cException->deleteOccurrence()) {
            sprintf(lastErrorMsg, ERR_OCCURRENCE_NOT_DELETED, cException->formatOriginalDate().c_str());
            LOG.error(lastErrorMsg);
            return 1;
        }
    }
    else {
        sprintf(lastErrorMsg, "Internal error: only 'deleted' exceptions can be saved. Trying to save a modified exception (date = %ls).", cException->formatOriginalDate().c_str());
        LOG.debug(lastErrorMsg);
        return 1;
        //
        // ****** SAVE OF MODIFIED EXCEPTIONS IS DISABLED ******
        ...
        //    sprintf(lastErrorMsg, ERR_OCCURRENCE_NOT_SAVED, cException->formatOriginalDate().c_str());
        //    LOG.error(lastErrorMsg);
        //    return 1;
        //}
    }
...
//                sprintf(lastErrorMsg, ERR_OCCURRENCE_NOT_DELETED, tmp);
//                LOG.error(lastErrorMsg);
//                continue;
//            }
//        }
