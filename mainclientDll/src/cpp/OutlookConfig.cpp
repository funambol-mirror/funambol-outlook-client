...
void OutlookConfig::createDefaultConfig() {
    ...
    for (int i=0; i<4; i++) {
        ...
        try {
            ...
            sprintf(lastErrorMsg, ERR_DEFAULT_SSCONFIG, name, e);
            safeMessageBox(lastErrorMsg);
        }
...
int OutlookConfig::setUniqueDevID() {
    ...
    if (!GetComputerNameA(computerName, &bufSize)) {
        ...
        sprintf (lastErrorMsg, ERR_MACHINE_NAME, code, msg);
        delete [] msg;
        return 1;
    }
    len += bufSize;
    ...
    if (!GetUserNameA(userName, &bufSize)) {
        code = GetLastError();
        msg = readSystemErrorMsg(code);
        sprintf (lastErrorMsg, ERR_USER_NAME, code, msg);
        delete [] msg;
        return 1;
    }
    len += bufSize;
...
int OutlookConfig::decryptPrivateData() {
    ...
    // Username
    char* decData = decryptData(accessConfig.getUsername());
    if (decData) {
        accessConfig.setUsername(decData);
        delete [] decData; decData = NULL;
    }
    // Password
    decData = decryptData(accessConfig.getPassword());
    if (decData) {
        accessConfig.setPassword(decData);
        delete [] decData; decData = NULL;
    }
    // Proxy username
    decData = decryptData(accessConfig.getProxyUsername());
    if (decData) {
        accessConfig.setProxyUsername(decData);
        delete [] decData; decData = NULL;
    }
    // Proxy password
    decData = decryptData(accessConfig.getProxyPassword());
    if (decData) {
...
void OutlookConfig::encryptPrivateData() {
    ...
    // Username
    char* encData = encryptData(accessConfig.getUsername());
    if (encData) {
        accessConfig.setUsername(encData);
        delete [] encData; encData = NULL;
    }
    // Password
    encData = encryptData(accessConfig.getPassword());
    if (encData) {
        accessConfig.setPassword(encData);
        delete [] encData; encData = NULL;
    }
    // Proxy username
    encData = encryptData(accessConfig.getProxyUsername());
    if (encData) {
        accessConfig.setProxyUsername(encData);
        delete [] encData; encData = NULL;
    }
    // Proxy password
    encData = encryptData(accessConfig.getProxyPassword());
    if (encData) {
