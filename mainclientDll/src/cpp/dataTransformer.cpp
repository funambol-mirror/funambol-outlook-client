        ...
        if (cEx->getDeleted()) {
            ...
            if (cItemNew) {
                ...
                if (!cItemNew->saveItem()) {
                    ...
                    sprintf(lastErrorMsg, ERR_ITEM_SAVE, APPOINTMENT, getSafeItemName(cItemNew).c_str(), cItemNew->getParentPath().c_str());
                    goto error;
                }
                delete cItemNew;
                cItemNew = NULL;
                ...
                sprintf(lastErrorMsg, ERR_ITEM_CREATE, APPOINTMENT, cItemNew->getParentPath().c_str());
                goto error;
            }
            ...
            cEx->setDeleted(TRUE);
            if (cItem->saveItem()) {
                sprintf(lastErrorMsg, ERR_ITEM_SAVE, L"recurring appointment", getSafeItemName(cItem).c_str(), cItem->getParentPath().c_str());
                goto error;
            }
...
int setRecurrenceExceptions(ClientItem* cItem, list<wstring> &excludeDates, list<wstring> &includeDates) {
    ...
    if (modified) {
        ...
        if (!filePath) {
            sprintf(lastErrorMsg, ERR_WFILE_OPEN, APPOINTMENT_FORCED_MODIFIED);
            goto error;
        }
