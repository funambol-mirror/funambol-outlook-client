...
int setScheduleTask (const char* frequency, const int dayNum, const int minNum) {
    ...
    if (FAILED(hr)) {
        if (hr == E_OBJECT_NOT_FOUND) {
            ...
            if (FAILED(hr)) {
                ...
                sprintf(lastErrorMsg, ERR_SCHED_NEWWORKITEM, hr, msg);
                delete [] msg;
                pITS->Release();
                goto error;
            }
            ...
            char* msg = readSystemErrorMsg(hr);
            sprintf(lastErrorMsg, ERR_SCHED_ACTIVATE, hr, msg);
            delete [] msg;
            pITS->Release();
            goto error;
        }
    ...
    if (newTask) {
        ...
        if (FAILED(hr)) {
            char* msg = readSystemErrorMsg(hr);
            sprintf(lastErrorMsg, ERR_SCHED_CREATE_TRIGGER, hr, msg);
            delete [] msg;
            goto error;
        }
    }
        ...
        if (FAILED(hr)) {
            char* msg = readSystemErrorMsg(hr);
            sprintf(lastErrorMsg, ERR_SCHED_GET_TRIGGER, hr, msg);
            delete [] msg;
            goto error;
        }
    }
    ...
    if (FAILED(hr)) {
        if (hr == E_INVALIDARG) {
            sprintf(lastErrorMsg, ERR_SCHED_SET_TRIGGER, hr, ERR_SCHED_INVALID_PARAM);
        }
        else {
            char* msg = readSystemErrorMsg(hr);
            sprintf(lastErrorMsg, ERR_SCHED_SET_TRIGGER, hr, msg);
            delete [] msg;
        }
        goto error;
    }
    ...
    if (FAILED(hr)) {
        char* msg = readSystemErrorMsg(hr);
        sprintf(lastErrorMsg, ERR_SCHED_QUERY_INTERFACE, hr, msg);
        delete [] msg;
        goto error;
    }
    pITask->Release();
    ...
    if (FAILED(hr)) {
        char* msg = readSystemErrorMsg(hr);
        sprintf(lastErrorMsg, ERR_SCHED_SAVE, hr, msg);
        delete [] msg;
        goto error;
    }
...
int getScheduleTask(bool* active, int* dayNum, int* minNum) {
    ...
    if ((hr & SCHED_S_TASK_HAS_NOT_RUN) || (hr != S_OK)) {
        sprintf(lastErrorMsg, DBG_SCHED_LAST_EXECUTION);
    ...
    if (FAILED(hr)) {
        char* msg = readSystemErrorMsg(hr);
        sprintf(lastErrorMsg, ERR_SCHED_GET_TRIGGER2, hr, msg);
        delete [] msg;
        goto error;
    }
...
int deleteScheduleTask() {
    ...
    if (FAILED(hr)) {
        if (hr == E_OBJECT_NOT_FOUND) {
            ...
            char* msg = readSystemErrorMsg(hr);
            sprintf(lastErrorMsg, ERR_SCHED_DELETE, hr, msg);
            delete [] msg;
            goto error;
        }
    }
...
ITaskScheduler* initScheduleInstance() {
    ...
    if (SUCCEEDED(hr)) {
        hr = CoCreateInstance(CLSID_CTaskScheduler,
                              ...
                              (void**) &pITS);
        if (FAILED(hr)) {
            sprintf(lastErrorMsg, "%s %s", ERR_SCHED_INIT_TASK, ERR_COM_CREATE_INSTANCE);
            return NULL;
        }
    }
    else {
        sprintf(lastErrorMsg, "%s %s", ERR_SCHED_INIT_TASK, ERR_COM_INITIALIZE);
        return NULL;
    }
