...
WCHAR* readAppDataPath() {
    ...
    if ( FAILED(SHGetFolderPath(NULL, 
        ...
        sprintf (lastErrorMsg, ERR_APPDATA_PATH, code, msg);
        delete [] msg;
        return NULL;
    }
...
int writeToFile(const wstring& content, const wstring& filePath, const WCHAR* mode) {
    ...
    if (!mode) {
        sprintf(lastErrorMsg, ERR_FILE_OPEN_MODE);
        return 1;
    }
    
    FILE* f;
    if ((f  = _wfopen(filePath.c_str(), mode)) == NULL) {
        sprintf(lastErrorMsg, ERR_WFILE_OPEN, filePath.c_str());
        return 1;
    }
    ...
    if (tmp) {
        if (fprintf(f, "%s", tmp) < 0) {
            sprintf(lastErrorMsg, ERR_WFILE_WRITE, filePath.c_str());
            return 1;
        }
        delete [] tmp;
    }
...
int writeToFile(const string& content, const string& filePath, const char* mode) {
    ...
    if ((f  = fopen(filePath.c_str(), mode)) == NULL) {
        sprintf(lastErrorMsg, ERR_FILE_OPEN, filePath.c_str());
        return 1;
    }

    if (fprintf(f, "%s", content.c_str()) < 0) {
        sprintf(lastErrorMsg, ERR_FILE_WRITE, filePath.c_str());
        return 1;
    }
...
int makeDataDirs() {
    ...
    if ( _wmkdir(tmpPath.c_str()) ) {
        _get_errno(&err);
        if (err != EEXIST) {
            sprintf(lastErrorMsg, ERR_DIR_CREATE, tmpPath.c_str());
            return 1;
        }
    }
